...
def validate_dataset(data_config: Dict, data_root: str = ".") -> Dict:
    ...
    if stats['issues']:
        ...
        logger.info(f"  Train/Val/Test: {stats['train_images']}/{stats['val_images']}/{stats['test_images']}")
        logger.info(f"  Classes: {stats['num_classes']} ({', '.join(stats['classes'])})")

    return stats


def create_data_yaml(output_path: str, train_dir: str, val_dir: str,
                    test_dir: Optional[str] = None, class_names: List[str] = None) -> str:
    """
    ...
    if class_names is None:
        ...
        class_names = [f"class_{i}" for i in range(len(class_names))] if class_names else ["object"]
...
def analyze_image_sizes(image_paths: List[str], sample_size: int = 1000) -> Dict:
    ...
    sizes = []
    sampled_paths = image_paths[:sample_size] if len(image_paths) > sample_size else image_paths
    ...
    return {
        ...
        'recommended_size': self._recommend_input_size(widths, heights)
    }


def _recommend_input_size(self, widths: List[int], heights: List[int]) -> int:
    """
...
def split_dataset(source_dir: str, output_dir: str, train_ratio: float = 0.7,
                 val_ratio: float = 0.2, test_ratio: float = 0.1, seed: int = 42) -> Dict[str, str]:
    """
