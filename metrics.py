...
def calculate_precision_recall(gt_boxes: List[List[float]], pred_boxes: List[List[float]],
                             gt_classes: List[int], pred_classes: List[int],
...
def calculate_map(gt_boxes_list: List[List[List[float]]], pred_boxes_list: List[List[List[float]]],
                 gt_classes_list: List[List[int]], pred_classes_list: List[List[int]],
                 num_classes: int, iou_thresholds: List[float] = [0.5]) -> Dict[str, float]:
    """
    ...
    for iou_thresh in iou_thresholds:
        ...
        for class_id in range(num_classes):
            ...
            for img_idx, (gt_boxes, pred_boxes, gt_classes, pred_classes) in enumerate(
                ...
                class_pred_confs.extend([0.5] * len(pred_boxes_class))  # Default confidence
            ...
            ap = calculate_ap(class_gt_boxes, class_pred_boxes, class_pred_confs, iou_thresh)
            ap_scores.append(ap)
...
def calculate_ap(gt_boxes: List[List[float]], pred_boxes: List[List[float]],
                pred_confs: List[float], iou_threshold: float = 0.5) -> float:
    """
...
def analyze_detection_errors(gt_boxes: List[List[float]], pred_boxes: List[List[float]],
                           gt_classes: List[int], pred_classes: List[int],
                           iou_threshold: float = 0.5) -> Dict[str, int]:
    """
...
def calculate_small_object_metrics(detections: List[Dict], min_area: int = 32*32) -> Dict[str, float]:
    """
    ...
    return {
        ...
        'small_object_recall': len([obj for obj in small_objects if obj['confidence'] > 0.5]) / small_count
    }
