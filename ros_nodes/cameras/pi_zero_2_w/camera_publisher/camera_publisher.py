...
def main(args=None):
    ...
    if args.pi_number and args.camera_type:
        ...
        # Detect Pi hardware capability for adaptive configuration
        try:
            with open('/proc/device-tree/model', 'r') as f:
                pi_model = f.read().strip()
            if 'Pi 5' in pi_model:
                pi_capability = 'high_end'
            elif 'Pi Zero 2 W' in pi_model:
                pi_capability = 'constrained'
            else:
                pi_capability = 'standard'
        except:
            pi_capability = 'standard'  # fallback
        ...
        if os.path.exists(config_file):
            ...
            # Apply hardware-adaptive modifications
            if 'camera' in config and pi_capability == 'high_end':
                # Pi 5 can handle higher resolutions and frame rates
                camera_config = config['camera']
                if camera_config.get('resolution') == 'half':
                    camera_config['resolution'] = 'full'  # Upgrade to full HD
                    camera_config['fps'] = min(camera_config.get('fps', 15) + 10, 30)  # Increase FPS
                if 'compression' in config['ros2']:
                    config['ros2']['compression']['target_frame_size_kb'] = 400  # Higher quality
                print(f"Pi 5 detected - upgraded to full HD resolution")
            elif pi_capability == 'constrained':
                # Ensure Pi Zero 2 W stays at optimized settings
                print(f"Pi Zero 2 W detected - using memory-optimized settings")
