...
def find_best_models():
    ...
    architectures = ["yolov8s", "mobilenet_vit", "efficientnet"]
    best_models = {}
    ...
    for arch in architectures:
        ...
        if arch_models:
            ...
            print(f"{arch}: Best model {best_model['name']} - mAP50: {best_model['mAP50']:.3f}")
...
def benchmark_best_models(best_models):
    ...
    trainer = ModelTrainer(
        {"training": {"device": "cuda" if __import__("torch").cuda.is_available() else "cpu"}}
    )
    ...
    benchmark_df = trainer.benchmark_rover_models(model_configs, data_yaml)
...
def show_final_results(benchmark_df):
    ...
    for i, (_, model) in enumerate(results_df.head(3).iterrows(), 1):
        ...
        print(f"   [PERFORMANCE] Speed: {model['fps']:.1f} FPS ({model['avg_latency_ms']:.1f}ms latency)")
    ...
    print(f"• Most Accurate: {best_accuracy['model']} ({best_accuracy['mAP50']:.3f} mAP50)")
    print(f"• Fastest: {best_speed['model']} ({best_speed['fps']:.1f} FPS)")
    print(f"• Smallest: {smallest_size['model']} ({smallest_size['model_size_mb']:.1f} MB)")

    # Save detailed results
    output_dir = Path("output")
    output_dir.mkdir(exist_ok=True)
    results_file = output_dir / "rover_final_benchmark_results.csv"
...
def main():
    ...
    # Show final results
    final_results = show_final_results(benchmark_df)

    print("\n[SUCCESS] ANALYSIS COMPLETE!")
    print("The rover optimization successfully trained and evaluated multiple models.")
    print("Check rover_final_benchmark_results.csv for detailed metrics.")
