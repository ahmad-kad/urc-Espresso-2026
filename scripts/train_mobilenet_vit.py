#!/usr/bin/env python3
"""
Train MobileNetViT object detection model
Custom training script for MobileNetViT + ViT detector
"""
...
def train_mobilenet_vit(data_yaml, epochs=50, batch_size=8, lr=0.001, device='cpu',
                       attention_layers=None, output_dir='output/models/mobilenet_vit'):
    """Train MobileNetViT detector"""
    ...
    model = create_mobilenet_vit_detector(num_classes=num_classes, attention_layers=attention_layers)
    model.to(device)
...
def main():
    parser = argparse.ArgumentParser(description='Train MobileNetViT detector')
    ...
    parser.add_argument('--output_dir', type=str, default='output/models/mobilenet_vit',
                       help='Output directory for model weights')

    args = parser.parse_args()

    # Define attention layers if requested
    attention_layers = ['fpn.0', 'fpn.3', 'fpn.6'] if args.attention else None
