...
# ModelEvaluator removed - functionality integrated into benchmark_models.py
...
def main():
    ...
    if not data_yaml:
        ...
        try:
            ...
            last_checkpoint = (
                checkpoint_dir / "last.pt" if checkpoint_dir.exists() else None
            )
    ...
    # Step 4: Convert to ONNX (FP32 and INT8)
    print("\nStep 4: Converting to ONNX (FP32, INT8)...")
    try:
        ...
        fp32_output = output_dir / f"{Path(model_path).stem}_fp32.onnx"
        convert_model(str(model_path), str(fp32_output), input_size=input_size)

        # Convert to INT8 ONNX
        int8_output = output_dir / f"{Path(model_path).stem}_int8.onnx"
        convert_onnx_to_int8(str(fp32_output), str(int8_output))

        onnx_paths = {"fp32": str(fp32_output), "int8": str(int8_output)}
    ...
    try:
        ...
        results_pt = evaluator_pt.benchmark_inference_speed(
            num_runs=50, input_size=input_size
        )
        print(
            f"    PyTorch - FPS: {results_pt['fps']:.2f}, Avg time: {results_pt['avg_inference_time']*1000:.2f}ms"
        )
        ...
        for precision in ["fp32", "fp16", "int8"]:
            if precision in onnx_paths:
                ...
                try:
                    ...
                    evaluator_onnx = ModelEvaluator(
                        onnx_paths[precision], model_type="onnx"
                    )
                    results_onnx = evaluator_onnx.benchmark_inference_speed(
                        num_runs=50, input_size=input_size
                    )
                    all_results[precision] = results_onnx
                    print(
                        f"    ONNX {precision.upper()} - FPS: {results_onnx['fps']:.2f}, Avg time: {results_onnx['avg_inference_time']*1000:.2f}ms"
                    )
                except Exception as e:
        ...
        print(
            f"{'PyTorch':<15} {results_pt['fps']:<12.2f} {results_pt['avg_inference_time']*1000:<15.2f} {pt_size:<12.2f}"
        )
        ...
        for precision in ["fp32", "fp16", "int8"]:
            if precision in all_results:
                ...
                print(
                    f"{'ONNX ' + precision.upper():<15} {results['fps']:<12.2f} {results['avg_inference_time']*1000:<15.2f} {onnx_size:<12.2f}"
                )
        ...
        if "fp32" in all_results:
            speedup_fp32 = (
                all_results["fp32"]["fps"] / results_pt["fps"]
                if results_pt["fps"] > 0
                else 0
            )
            print(f"\nSpeedup vs PyTorch:")
            ...
            if "fp16" in all_results:
                speedup_fp16 = (
                    all_results["fp16"]["fps"] / results_pt["fps"]
                    if results_pt["fps"] > 0
                    else 0
                )
            ...
            if "int8" in all_results:
                speedup_int8 = (
                    all_results["int8"]["fps"] / results_pt["fps"]
                    if results_pt["fps"] > 0
                    else 0
                )
        ...
        if "fp32" in onnx_paths:
            ...
            if "fp16" in onnx_paths:
                ...
                print(
                    f"  FP16: {reduction:.1f}% smaller ({fp16_size:.2f} MB vs {fp32_size:.2f} MB)"
                )
            if "int8" in onnx_paths:
                int8_size = Path(onnx_paths["int8"]).stat().st_size / (1024 * 1024)
                reduction = (1 - int8_size / fp32_size) * 100
                print(
                    f"  INT8: {reduction:.1f}% smaller ({int8_size:.2f} MB vs {fp32_size:.2f} MB)"
                )
