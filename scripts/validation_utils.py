...
class DatasetVisualizer:
    ...
    def create_class_grid(self, output_path="../output/class_samples_grid.jpg", grid_size=(5, 1)):
        ...
        for col, class_id in enumerate(range(len(self.class_names))):
            if class_id in selected_images:
                ...
                try:
                    ...
                    draw.text((text_x, text_y), title, fill=(0, 0, 0), font=font, align='center')
                    ...
                    x_pos = col * (image_size[0] + padding) + padding
                    y_pos = padding
                    draw.rectangle([x_pos, y_pos, x_pos + image_size[0], y_pos + image_size[1]],
                                 fill=(200, 200, 200), outline=(0, 0, 0))
                    draw.text((x_pos + 10, y_pos + image_size[1]//2),
                             f"No image\n{self.class_names[class_id]}",
                             fill=(0, 0, 0), font=font)

        # Add main title
        title_font = ImageFont.truetype("arial.ttf", 24) if font != ImageFont.load_default() else font
        ...
        draw.text((text_x, grid_height - 30), main_title, fill=(0, 0, 0), font=title_font)
    ...
    def show_class_samples(self, output_path="../output/detailed_class_samples.jpg"):
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            if len(parts) >= 5:
                                ...
                                image_path = self.train_images / f"{image_name}.jpg"
                                if image_path.exists() and '_aug_' not in image_name:
                                    class_samples[class_id].append((image_path, image_name))
            except:
                continue
        ...
        grid_width = grid_cols * (image_size[0] + padding) + padding
        grid_height = grid_rows * (image_size[1] + class_title_height + sample_title_height + padding) + padding
        ...
        for row, class_id in enumerate(sorted(self.class_names.keys())):
            ...
            title_y = row * (image_size[1] + class_title_height + sample_title_height + padding) + padding
            ...
            for col, (image_path, image_name) in enumerate(samples):
                try:
                    ...
                    # Add sample name
                    sample_name = image_name[:20] + "..." if len(image_name) > 20 else image_name
                    ...
                    draw.text((name_x, name_y), sample_name, fill=(0, 0, 0), font=sample_font)
...
class PerformanceAnalyzer:
    ...
    def create_per_class_analysis(self, performance_data=None):
        ...
        if performance_data is None:
            ...
            performance_data = {
                'YOLOv8s Baseline': {
                    'ArUcoTag': {'Precision': 0.814, 'Recall': 0.617, 'mAP50': 0.628, 'mAP50-95': 0.422},
                    'Bottle': {'Precision': 0.892, 'Recall': 0.878, 'mAP50': 0.897, 'mAP50-95': 0.627},
                    'BrickHammer': {'Precision': 0.977, 'Recall': 0.825, 'mAP50': 0.927, 'mAP50-95': 0.732},
                    'OrangeHammer': {'Precision': 0.872, 'Recall': 0.975, 'mAP50': 0.983, 'mAP50-95': 0.798}
                }
            }
    ...
    def _create_performance_plots(self, df):
        ...
        plt.tight_layout()
        plt.savefig('../output/per_class_performance_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()

        print("Performance plots saved to: ../output/per_class_performance_comparison.png")

    def create_model_comparison_tables(self, results_path="../output/benchmark_results"):
        ...
        try:
            ...
            performance_df = pd.DataFrame({
                'Model': ['YOLOv8s', 'YOLOv8m', 'EfficientNet', 'MobileNet-ViT'],
        ...
        # Save to file
        performance_df.to_csv(results_path / "model_comparison_overall.csv", index=False)
    ...
    def _create_detailed_comparison_tables(self, df, output_path):
        ...
        print(f"Best mAP50: {sorted_df.iloc[0]['Model']} ({sorted_df.iloc[0]['mAP50']:.3f})")
        print(f"Best mAP50-95: {sorted_df.loc[sorted_df['mAP50-95'].idxmax()]['Model']} ({sorted_df['mAP50-95'].max():.3f})")
        print(f"Best Precision: {sorted_df.loc[sorted_df['Precision'].idxmax()]['Model']} ({sorted_df['Precision'].max():.3f})")
        print(f"Best Recall: {sorted_df.loc[sorted_df['Recall'].idxmax()]['Model']} ({sorted_df['Recall'].max():.3f})")
        print()
        ...
        with open(output_path / "performance_comparison_table.txt", 'w') as f:
            ...
            for idx, row in sorted_df.iterrows():
                f.write(".3f")
            f.write("\n\nPerformance Summary:\n")
            f.write(f"Best mAP50: {sorted_df.iloc[0]['Model']} ({sorted_df.iloc[0]['mAP50']:.3f})\n")
            f.write(f"Best mAP50-95: {sorted_df.loc[sorted_df['mAP50-95'].idxmax()]['Model']} ({sorted_df['mAP50-95'].max():.3f})\n")
            f.write(f"Best Precision: {sorted_df.loc[sorted_df['Precision'].idxmax()]['Model']} ({sorted_df['Precision'].max():.3f})\n")
            f.write(f"Best Recall: {sorted_df.loc[sorted_df['Recall'].idxmax()]['Model']} ({sorted_df['Recall'].max():.3f})\n")
...
def main():
    ...
    parser.add_argument('--action', choices=[
        'class_grid', 'detailed_samples', 'analyze_performance', 'comparison_tables'
    ...
    parser.add_argument('--results_path', default='../output/benchmark_results', help='Path to results data')
