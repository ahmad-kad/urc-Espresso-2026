...
def plot_prediction_results(image: np.ndarray, detections: List[Dict],
                          class_names: List[str], output_path: Optional[str] = None,
                          conf_threshold: float = 0.5) -> str:
    """
    ...
    try:
        ...
        for detection in detections:
            ...
            cv2.rectangle(img, (int(x1), int(y1) - text_height - 5),
...
def plot_performance_comparison(models_results: Dict[str, Dict],
        ...
        for metric in metrics:
            ...
            for model_name in model_names:
                if 'metrics' in models_results[model_name]:
                    value = models_results[model_name]['metrics'].get(metric.replace('@', ''), 0)
...
def create_detection_mosaic(images: List[np.ndarray], detections: List[List[Dict]],
                           class_names: List[str], grid_size: Tuple[int, int] = (2, 3),
                           output_path: Optional[str] = None) -> str:
    """
...
def plot_realtime_performance(fps_data: List[float], output_path: Optional[str] = None) -> str:
    """
